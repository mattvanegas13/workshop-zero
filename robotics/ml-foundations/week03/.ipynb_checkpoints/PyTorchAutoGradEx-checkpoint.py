...
class Variable:
    ...
    def __mul__(self, rhs: 'Variable') -> 'Variable':
        ...
        gradient_tape.append(TapeEntry(inputs=inputs, outputs=outputs, propogate=propagate)) 
        return r
    ...
    def __add__(self, rhs: 'Variable') -> 'Variable':
        ...
        inputs = [self.name, rhs.name]
        outputs = [self.name]
        def propogate(dL_doutputs:List[Variable]):
        ...
        gradient_tape.append(TapeEntry[inputs=inputs, output=outputs, propogate=propogate])
        return r
    ...
    def sum(self, name: Optional[str]=None) -> 'Variable':
        ...
        r = Variable(torch.sum(self.value))
        def propogate(dL_doutputs:List[Variable]):
        ...
        gradient_tape.append(TapeEntry(inputs=[self.name], outputs=[r.name], propogate=propogate))
        return r
...
class TapeEntry(NamedTuple):
    ...
    """
    propogate: 'Callable[List[Variable], List[Variable]]'
...
def grad(L, desired_results: List[Variable]) -> List[Variable]:
    ...
    for entry in reversed(gradient_tape):
        ...
        dL_dinputs = entry.propogate(dL_doutputs)
